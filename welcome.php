...
<div class="container py-4">
  ...
  <div class="card mb-3">
    <div class="card-body">
      ...
      <div class="d-flex flex-wrap mt-2" style="gap:8px;">
        <button id="save" class="btn btn-primary" type="button">保存（ローカル）</button>
  ...
  <div class="card">
    <div class="card-body">
      ...
      <div class="small">※保存先はこの端末/このブラウザの localStorage です（別端末や別ブラウザには同期されません）。</div>
    </div>
  </div>
</div>
...
<script>
(() => {
  const KEY = "electric_meter_readings_v1";
  ...
  function renderHistory() {
    const items = loadHistory().sort((a,b) => a.timestamp - b.timestamp);
    ...
    historyBody.querySelectorAll("button[data-del]").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const items = loadHistory().filter(x => x.timestamp !== ts);
        saveHistory(items);
        renderHistory();
      });
    });
  ...
  function saveReading() {
    ...
    saveHistory(items);
    renderHistory();
    setStatus("保存しました（ローカル）。");
  }

  function exportCsv() {
    const items = loadHistory().sort((a,b) => a.timestamp - b.timestamp);
    const lines = ["timestamp,datetime,reading"];
    for (const x of items) {
      lines.push(`${x.timestamp},${fmtDate(x.timestamp)},${x.reading}`);
    }
    const blob = new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "electric_meter_history.csv";
    a.click();
